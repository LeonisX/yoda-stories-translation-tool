...
uses
  Windows, Messages, SysUtils, Classes, Controls, Forms, MPHexEditor, ActiveX, Graphics, Printers,
  {$IFDEF DELPHI6UP}{$IFDEF USESTDACTIONS}StdActns, Clipbrd,{$ENDIF}{$ENDIF} ShlObj, Menus;
...
type
  ...
  *)
  ...
  TMPHQueryPublicPropertyEvent = procedure(Sender: TObject; const PropertyName: string;
...
begin
  ...
  Result := (not (ReadOnlyView (*or NoSizeChange*))) and
    ...
    Point(0, 0), LintEffect, oleClipboard) = S_OK);
...
function TMPHexEditorEx.InsertOLEData(const dataObj: IDataObject; const
  ...
  Operation: TMPHOLEOperation): HRESULT;
var
  LRecStg: TStgMedium;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (lIntLoop = Length(Result.Text)) or (not (pfUseBackgroundColor in FFlags)) then
        begin
          if (lIntLoop = Length(Result.Text)) and (pfUseBackgroundColor in FFlags) then
            Bold := False;
          Fore := FEditor.Font.Color;
          Back := FEditor.Colors.Background;
        end
        ...
        begin
          if lBold then
          begin
            if (pfUseBackgroundColor in FFlags) and not (pfMonochrome in FFlags) then
              Bold := False;
            Fore := FEditor.Colors.CurrentOffset;
            Back := FEditor.Colors.CurrentOffsetBackground;
          end
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (FEditor.UsedRulerBytesPerUnit <> 2) and ((((LIntLoop-1) * 2) mod FEditor.UsedRulerBytesPerUnit) = 0) then
            AddSpacer;
        end;
      end;
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if pfUseBackgroundColor in FFlags then
              FCanvas.Brush.Color := LRecTextAttr.Attributes[LIntLoop - 1].Back
            else
              if LRecTextAttr.Attributes[LIntLoop - 1].Fore = clWhite then
                FCanvas.Font.Color := clBlack;
          end;

          if FFlags * [pfSelectionBold, pfSelectionOnly] = [pfSelectionBold] then
          begin
            if FFlags * [pfMonochrome, pfUseBackgroundColor] = [pfUseBackGroundColor] then
            begin
              FCanvas.Font.Style := [];
              if LRecTextAttr.Attributes[LIntLoop - 1].Bold then
              begin
          ...
          if FEditor.UnicodeChars then
            while (FCanvas.Font.Size > 1) and GetTextExtentPoint32W(FCanvas.Handle, @LRecTextAttr.Text[LIntLoop], 1, LrecSize) and (LRecSize.cx > (LRectOut.Right - LRectOut.Left)) do
            FCanvas.Font.Size := FCanvas.Font.Size -1;
          ExtTextOutW(FCanvas.Handle, LIntLeft, LIntY, ETO_CLIPPED or ETO_OPAQUE, @LRectOut, @LRecTextAttr.Text[LIntLoop], 1, nil);
          if FEditor.UnicodeChars then
